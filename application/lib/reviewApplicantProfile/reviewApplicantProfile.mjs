...
export const handler = async (event) => {
     ...
     try {
          ...
          const appliedJobs = await runQuery (`SELECT j.jobName, ja.jobAppId, GROUP_CONCAT(js.jobSkill ORDER BY js.jobSkill SEPARATOR ', ') AS skillsNeeded FROM JobApplication ja JOIN Jobs j ON ja.jobApp_jobId_FK = j.jobId JOIN JobSkills js  ON js.jobSkill_jobId_FK = j.jobId WHERE ja.jobApp_appId_FK = ? AND ja.offered = 0 AND ja.hired = 0 AND ja.rejectedByApplicant = 0 AND ja.withdrawn=0 GROUP BY j.jobName;`,[appId])
          const offeredJobs=await runQuery(`SELECT j.jobName, ja.jobAppId, GROUP_CONCAT(js.jobSkill ORDER BY js.jobSkill SEPARATOR ', ') AS skillsNeeded FROM JobApplication ja JOIN Jobs j ON ja.jobApp_jobId_FK = j.jobId JOIN JobSkills js ON js.jobSkill_jobId_FK = j.jobId WHERE ja.jobApp_appId_FK = ? AND ja.offered = 1 AND ja.rejectedByApplicant = 0 AND ja.hired = 0 GROUP BY j.jobName;`,[appId])
          const rejectedJobs=await runQuery(`SELECT j.jobName, ja.jobAppId, GROUP_CONCAT(js.jobSkill ORDER BY js.jobSkill SEPARATOR ', ') AS skillsNeeded FROM JobApplication ja JOIN Jobs j ON ja.jobApp_jobId_FK = j.jobId JOIN JobSkills js ON js.jobSkill_jobId_FK = j.jobId WHERE ja.jobApp_appId_FK = ? AND ja.rejectedByApplicant = 1 GROUP BY j.jobName;`,[appId])
          const acceptedJobs=await runQuery(`SELECT j.jobName, ja.jobAppId, GROUP_CONCAT(js.jobSkill ORDER BY js.jobSkill SEPARATOR ', ') AS skillsNeeded FROM JobApplication ja JOIN Jobs j ON ja.jobApp_jobId_FK = j.jobId JOIN JobSkills js ON js.jobSkill_jobId_FK = j.jobId WHERE ja.jobApp_appId_FK = ? AND ja.hired = 1 GROUP BY j.jobName;`,[appId])
